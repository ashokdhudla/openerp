			...
			for line in order.invoice_line:
				...
				if line.cst_vat_tax:
					if line.cst_vat_tax != False:
						tax = tax_obj.search(cr,uid,[('name','=',line.cst_vat_tax)])
					tax_object = tax_obj.browse(cr,uid,tax)
					for c in tax_obj.compute_all(cr, uid, tax_object,line.price_unit * (1-(line.discount or 0.0)/100.0), line.quantity, line.product_id, line.invoice_id.partner_id)['taxes']:
						val += c.get('amount', 0.0)
...
class invoice_line_new(osv.osv):
	...
	def _amount_line_new(self, cr, uid, ids, field_name, arg, context=None):
		...
		for line in self.browse(cr, uid, ids, context=None):
			...
			if line.cst_vat_tax:
				...
				taxes = tax_obj.compute_all(cr, uid,tax_object, price, line.quantity, line.product_id, line.invoice_id.partner_id)
				val1 = taxes['taxes'][0]['amount']
                                #total_equal1 = res[line.id]['price_subtotal'] + val1
			...
			if line.service_tax:
				...
				val2 = taxes['taxes'][0]['amount']
                                #total_equal2 = res[line.id]['price_subtotal']+ val2
			if  line.cst_vat_tax == False and line.service_tax == False:
				total_equal3 = res[line.id]['price_subtotal']
			#else:
				#price = res[line.id]['price_subtotal']
				#print ('Price for T0tal Amount :'),price
				#taxes = tax_obj.compute_all(cr, uid,line.tax_id, price, 1.0, line.product_id, line.order_id.partner_id)
			#cur = line.order_id.pricelist_id.currency_id
			#res[line.id] = cur_obj.round(cr, uid, cur, (price-(taxes['total_included']-taxes['total'])))
	...
	_columns = {
		...
		'cst_vat_tax':fields.char('CST/VAT Tax',size=200),
		'service_tax':fields.char('Service Tax',size=200),
