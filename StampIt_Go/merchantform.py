...
class stamp_merchant(osv.osv):
	...
	_columns = {
		...
		'stamp_value' : fields.integer('Stamp Value'),
		...
		'tc_id' : fields.one2many('terms.conditions','terms_id'),
...
class terms_conditions(osv.osv):
	...
	_description = 'Loyality Terms and Conditions'
	_rec_name = 'loyalty_card_no'
	_columns = {
		'terms_id' : fields.many2one('stamp.merchant'),
		'loyalty_card_no' : fields.integer('Loyality Card No',required=True),
		'terms_condition' : fields.text('T&C'),
	}
...
class current_offer_details(osv.osv):
	...
	_columns= {
		...
		'current_reward_name' : fields.char('Savings',size=50,required=True),
		'desc_reward' : fields.text('Offer Description'),
		'stamp_no' : fields.char('Offer Name',required=True),
	}
...
class offers_birthday(osv.osv):
	...
	_columns = {
		...
		'birthday_reward_name' : fields.char('Offer Name',required=True),
		'birthday_reward_description' : fields.text('Offer Description'),
...
class promo_offer_details(osv.osv):
	...
	_columns= {
		...
		'promo_reward_name' : fields.char('Offer Name',size=50,required=True),
		'promo_desc_reward' : fields.text('Offer Description'),
		# 'stamp_no' : fields.char('Stamp No',required=True),
	}
promo_offer_details()
