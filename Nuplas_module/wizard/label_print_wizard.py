...
class label_print_wizard(osv.osv_memory):
    ...
    def print_report(self, cr, uid, ids, context=None):
        print "hello this is wizard for printing..........."
        if context is None:
            print "context none"
            context = {}
        x = context.get('active_ids')
        print "active idssss",x
        pkg_obj = self.pool.get('packaging.board').browse(cr,uid,x)
        print "pkg_obj",pkg_obj[0].customer.id
        lp_id = self.pool.get('label.print').search(cr,uid,[('customer_id','=',pkg_obj[0].customer.id)])
        print lp_id[0]
        context.update({"label_print":lp_id[0]});
        y = context.get('label_print')
        print "label printtttttttttttttttt",y
        if not context.get('active_ids'):
            print "faslseeeeeeeeeee"
            return False
        total_record = len(context.get('active_ids', []))
        print "total_records111112222222332",total_record
        datas = {}
